...
-- Defaults GLOBAIS (empresa_id NULL) — semeados UMA vez, só em instalação nova.
-- Guard NOT EXISTS: se JÁ existir qualquer default global (instalação existente, ou o
-- usuário já apagou algum), o bloco NÃO roda. Assim o sistema deixa de RECRIAR, a cada
-- deploy, tipos que o usuário removeu (era isso que fazia o "Reunião" voltar sozinho).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM "agenda_tipos" WHERE "id" LIKE 'agtipo_g_%') THEN
    ...
    VALUES
      ...
      ('agtipo_g_reuniao_lead', NULL,'Reunião com Lead', '#fb7185','#e11d48','#ffffff', true,  false, false, false, false, '{}', true, now(), now()),
