...
### Entrega
- [x] **Commit de extração** ✅ = `0a668cb` (pdf-extract, extract-tabela, service async,
  page.tsx conversão .pdf, 2 docs).
- [ ] **Commit de modelos** (D/C SINAL + parser marcador + zero-skip + itens 3/4/6):
  apply-model, parsers, model-editor, version-diff, version-overview, treatment-definition,
  types, + hunks de page.tsx (popup competência). Falta.
- [ ] **Gate final** + **testes de UI** dos itens de modelo antes do commit.
