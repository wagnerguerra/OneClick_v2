...
export class AssinaturaPdfService {
  ...
  async assinar(input: {
    ...
    let pdf: Buffer<ArrayBuffer> = Buffer.from(input.pdfBase64, 'base64')
    if (input.area) pdf = await this.carimbar(pdf, input.area, cert.titular, cert.documento)
  ...
  /**
   * Desenha o carimbo na área escolhida.
   *
   * Feito ANTES de assinar, de propósito: alterar o PDF depois invalidaria a
   * assinatura — é exatamente o que ela existe para detectar.
   */
  private async carimbar(pdf: Buffer, area: AreaAssinatura, titular: string, documento: string): Promise<Buffer<ArrayBuffer>> {
