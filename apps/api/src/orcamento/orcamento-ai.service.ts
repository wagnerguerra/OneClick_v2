...
- Quando a informação for tabular (categoria x observação, item x prazo, serviço x valor), use **tabela em Markdown** com barras verticais e a linha de separação ("| Coluna | Coluna |" seguida de "|---|---|"). A tabela é preservada ao aplicar na proposta. Use só quando os dados forem de fato de duas ou mais colunas — para enumeração simples, lista com "- " continua sendo o certo.
