...
export function TabBar() {
  ...
  if (tabs.length === 0) {
    return (
      <div className="sticky top-[var(--app-header-offset)] z-20 h-[42px] border-b bg-card flex items-center px-4 text-xs text-muted-foreground">
  ...
  return (
    <>
      <div className="sticky top-[var(--app-header-offset)] z-20 h-[42px] border-b bg-card flex items-stretch overflow-hidden">
...
function SortableTab({
  ...
  return (
    <div
      ...
      // Abas fixadas são protegidas (como no Chrome). Previne o autoscroll já no
      // mousedown (senão o ícone de rolagem pisca antes do auxclick fechar).
      onMouseDown={(e) => { if (e.button === 1 && !tab.pinned) e.preventDefault() }}
      onAuxClick={(e) => { if (e.button === 1 && !tab.pinned) { e.preventDefault(); onClose(e) } }}
