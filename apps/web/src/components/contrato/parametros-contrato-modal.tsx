...
import { FileText, Save, Loader2, Search as SearchIcon } from 'lucide-react'
import {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeaderIcon icon={FileText} color="sky">
          <DialogTitle>Parâmetros do Contrato</DialogTitle>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col justify-end gap-2 pb-1">
                    <label className="flex items-center gap-2 text-xs cursor-pointer">
                      <Checkbox checked={params.permanente}
                        onCheckedChange={(v) => setParams(p => ({ ...p, permanente: v === true, ...(v === true ? { dataFim: '' } : {}) }))} />
                      Contrato permanente (sem prazo)
                    </label>
                    <label className="flex items-center gap-2 text-xs cursor-pointer">
                      <Checkbox checked={params.gestaoIgnorar}
                        onCheckedChange={(v) => setParams(p => ({ ...p, gestaoIgnorar: v === true }))} />
        ...
        <DialogFooter className="sm:justify-between">
          <Button type="button" variant="outline" size="sm" onClick={obterSugeridos} disabled={buscando || loading}>
            ...
            {buscando ? 'Consultando SCI...' : 'Obter parâmetros iniciais'}
          </Button>
