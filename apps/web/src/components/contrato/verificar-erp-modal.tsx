...
const INDICADORES = [
  ...
  { key: 'vidas', label: 'Funcionários' },
] as const

const LABEL_INDICADOR: Record<string, string> = Object.fromEntries(INDICADORES.map(i => [i.key, i.label]))
...
export function VerificarErpModal({ clienteId, open, onOpenChange, subtitulo, onSincronizado }: VerificarErpModalProps) {
  ...
  async function consultar() {
    ...
    try {
      ...
      alerts.error('Erro SCI', mensagemErro(e, 'Não foi possível consultar o ERP.'))
    } finally { setLoading(false) }
  }

  // Não deixa fechar no meio da consulta (overlay, ESC, X e Fechar passam por aqui).
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeaderIcon icon={ExternalLink}>
          <DialogTitle>Verificar no ERP (SCI)</DialogTitle>
          {subtitulo && <DialogDescription>{subtitulo}</DialogDescription>}
        </DialogHeaderIcon>

        {/* Filtros — fixos; só o resultado (DialogBody) rola. */}
        ...
        <DialogBody>
          {loading ? (
            ...
            <div className="space-y-4">
              <div className={cn('rounded px-3 py-2 text-xs', BADGE.emerald)}>
                Período: {(resultado.periodo as Record<string, string>)?.datai} a {(resultado.periodo as Record<string, string>)?.dataf}
                {' | '}CNPJ: {resultado.cnpj as string}
                {' · '}os períodos consultados ficam salvos e alimentam o farol.
              </div>
              {selecionados.map((ind) => {
                const rotulo = LABEL_INDICADOR[ind] ?? ind
                ...
                if (!rows || rows.length === 0) return (
                  <div key={ind} className="text-xs text-muted-foreground">
                    <strong className="text-foreground">{rotulo}</strong>: Sem dados no período
                  </div>
                )
                ...
                return (
                  <div key={ind}>
                    <div className="flex items-center justify-between mb-1">
                      <h5 className="text-xs font-semibold text-foreground">{rotulo}</h5>
                      <div className="text-[10px] text-muted-foreground">
                        Total: <strong>{fmt(total)}</strong>&nbsp;|&nbsp;Média: <strong>{fmt(media)}</strong>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-[11px]">
                        <thead>
                          <tr className="border-b border-border/40">
                            <th className="text-left py-1 pr-3 font-semibold text-muted-foreground">Mês/Ano</th>
        ...
        <DialogFooter>
          <DialogClose asChild>
            <Button type="button" variant="outline" size="sm" disabled={loading}>Fechar</Button>
          </DialogClose>
        </DialogFooter>
      </DialogContent>
    </Dialog>
