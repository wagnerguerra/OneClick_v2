...
/**
 * Dashboard Financeiro — versão do CLIENTE (link público por token).
 *
 * É a mesma informação do `/bi-faturamento`, sem nada de aplicação: não há
 * menu lateral, barra do sistema, avatar nem navegação para outros módulos.
 * Quem abre isto recebeu um link do contador e quer ver os próprios números.
 *
 * ## Padrão visual
 *
 * Segue o LuminAux, como o resto do v2 desde 20/08: Plus Jakarta Sans (global,
 * herdada do layout raiz), hierarquia por PESO e COR em vez de tamanho, títulos
 * compactos, `rounded-lg`, superfícies em `bg-card` sobre `bg-background`.
 *
 * Isto substituiu uma paleta inteira de cinzas cravados (`bg-gray-50`,
 * `bg-white`, `text-gray-500`, `#0ea5e9`) que ignorava os tokens do tema — era
 * a única tela do sistema que não acompanhava o dark mode.
 */
...
export default function BiPublicContent() {
  ...
  // Meses — o filtro existia só na tela interna, e o cliente ficava sem recorte
  // nenhum: via sempre o ano inteiro, com os meses futuros desenhados zerados.
    ...
    Promise.all(
      anosSelecionados.map(async a => {
        const [k, an] = await Promise.all([
          trpc.biPublic.balanceteKpis.query({ token, ano: a, meses: mesesParam }).catch(() => null),
          trpc.biPublic.balanceteAnalise.query({ token, ano: a, meses: mesesParam }).catch(() => null),
        ])
  ...
  }, [token, ctx, anosSelecionados, mesesParam])
  ...
  if (error) return (
    <div className="flex h-screen items-center justify-center bg-background px-4">
      <div className={cn(CARD, 'flex max-w-md flex-col items-center gap-3 p-8 shadow-sm')}>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      {/* ── Identificação ──
          ...
          escritório e o nome da empresa. Nada de navegação. */}
      ...
      <div className="border-b border-border bg-card">
        <div className="mx-auto flex w-full max-w-screen-xl flex-wrap items-center gap-x-6 gap-y-3 px-4 py-3 sm:px-8">
      ...
      {/* ── Abas ── */}
      <div className="border-b border-border bg-card">
        <div className="mx-auto flex w-full max-w-screen-xl gap-0 overflow-x-auto px-4 sm:px-8">
      ...
      <main className="mx-auto w-full max-w-screen-xl flex-1 px-4 py-6 sm:px-8">
