...
export default function AssinaturaTemplatePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const payload = {
        ...
        logoOverrideUrl: template.logoOverrideUrl,
        ...
        photoOffsetX: template.photoOffsetX,
        photoOffsetY: template.photoOffsetY,
        textOffsetX: template.textOffsetX,
        textOffsetY: template.textOffsetY,
        // Sanitiza lineOffsets — só inclui chaves com x ou y setados
        lineOffsets: Object.fromEntries(
          Object.entries(template.lineOffsets ?? {})
  ...
  async function handleLogoUpload(file: File) {
    ...
    try {
      setField('logoOverrideUrl', await uploadImage(file, 2))
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader className="border-b border-border/60 px-5 py-3 flex flex-row items-center gap-2">
                ...
                <h3 className="text-sm font-semibold flex-1">Logo (override do template)</h3>
                <label className="flex items-center gap-2 text-[12px] font-semibold cursor-pointer">
                  <input type="checkbox" checked={template.showLogo} onChange={e => setField('showLogo', e.target.checked)} className="h-4 w-4" />
                  Exibir logo
                </label>
              </CardHeader>
              <CardContent className={`p-5 space-y-3 ${!template.showLogo ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex items-center gap-3">
                  {template.logoOverrideUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={resolveAssetUrl(template.logoOverrideUrl)} alt="Logo override" className="h-16 w-auto max-w-[180px] rounded border border-border object-contain bg-white p-1.5" />
                  ) : (
                    <div className="h-16 w-32 rounded border-2 border-dashed border-border bg-muted/40 flex items-center justify-center text-[11px] text-muted-foreground">Usa logo padrão da empresa</div>
                  )}
                  <div className="flex flex-col gap-2">
                    <Button size="sm" variant="outline" className="gap-1.5" onClick={() => logoInputRef.current?.click()} disabled={uploadingLogo}>
                      {uploadingLogo ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}
                      Subir override
                    </Button>
                    {template.logoOverrideUrl && (
                      <Button size="sm" variant="ghost" className="gap-1.5 text-rose-600" onClick={() => setField('logoOverrideUrl', null)}>
                        <X className="h-3.5 w-3.5" /> Remover
                      </Button>
                    )}
                ...
                <div className="space-y-1.5">
                  <Label className="text-[12px] font-semibold">Posição do logo</Label>
                  <Select value={template.logoPosition} onValueChange={v => setField('logoPosition', v as 'left' | 'right')}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="left">Esquerda (antes da foto)</SelectItem>
                      <SelectItem value="right">Direita (padrão)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </CardContent>
              ...
              <CardContent className="p-5 space-y-4">
                <div>
                  <div className="text-[12px] font-semibold mb-2 flex items-center gap-2">
                    <ImageIcon className="h-3.5 w-3.5 text-muted-foreground" /> Foto do usuário
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-[11px] text-muted-foreground">Horizontal: {template.photoOffsetX}px</Label>
                      <input type="range" min={-200} max={300} value={template.photoOffsetX} onChange={e => setField('photoOffsetX', Number(e.target.value))} className="w-full" />
                    </div>
                    <div>
                      <Label className="text-[11px] text-muted-foreground">Vertical: {template.photoOffsetY}px</Label>
                      <input type="range" min={-200} max={300} value={template.photoOffsetY} onChange={e => setField('photoOffsetY', Number(e.target.value))} className="w-full" />
                    </div>
                  </div>
                </div>
                <div className="pt-3 border-t border-border/40">
                  <div className="text-[12px] font-semibold mb-2 flex items-center gap-2">
                    <Code2 className="h-3.5 w-3.5 text-muted-foreground" /> Bloco de textos
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-[11px] text-muted-foreground">Horizontal: {template.textOffsetX}px</Label>
                      <input type="range" min={-200} max={300} value={template.textOffsetX} onChange={e => setField('textOffsetX', Number(e.target.value))} className="w-full" />
                    </div>
                    <div>
                      <Label className="text-[11px] text-muted-foreground">Vertical: {template.textOffsetY}px</Label>
                      <input type="range" min={-200} max={300} value={template.textOffsetY} onChange={e => setField('textOffsetY', Number(e.target.value))} className="w-full" />
                    </div>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground">
                  Range 0-300px (padding adicional). Negativos não funcionam em email — pra mover algo "pra esquerda" do limite, ajuste o outro lado.
                </p>
              </CardContent>
            </Card>

            {/* Distanciamento por linha */}
            <Card>
              <CardHeader className="border-b border-border/60 px-5 py-3 flex flex-row items-center gap-2">
                <Code2 className="h-4 w-4 text-muted-foreground" />
                <h3 className="text-sm font-semibold">Distanciamento por linha</h3>
              </CardHeader>
              <CardContent className="p-5 space-y-3">
                {([
                  ...
                  const offset = (template.lineOffsets ?? {})[key] ?? {}
                  const x = offset.x ?? 0
                  const y = offset.y ?? 0
                  const setOffset = (axis: 'x' | 'y', value: number) =>
                    setField('lineOffsets', {
                      ...(template.lineOffsets ?? {}),
                      [key]: { ...(template.lineOffsets ?? {})[key], [axis]: value },
                    })
                  return (
                    <div key={key} className="grid grid-cols-[80px_1fr_1fr] gap-3 items-center">
                      <div className="text-[12px] font-semibold">{label}</div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">X: {x}px</Label>
                        <input type="range" min={-200} max={300} value={x} onChange={e => setOffset('x', Number(e.target.value))} className="w-full" />
                      </div>
                      <div>
                        <Label className="text-[10px] text-muted-foreground">Y: {y}px</Label>
                        <input type="range" min={-200} max={300} value={y} onChange={e => setOffset('y', Number(e.target.value))} className="w-full" />
                      </div>
                    </div>
                  )
                })}
                <p className="text-[10px] text-muted-foreground pt-2 border-t border-border/40">
                  Range -200 a +300px. <strong>Aviso:</strong> margin negativo funciona em Gmail/Apple Mail/iPhone mas é
                  ignorado em Outlook desktop. Pra fallback robusto, use só valores positivos.
                </p>
              </CardContent>
            </Card>
