...
export default function MontarRelatorioPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* A largura da coluna e que carrega o movimento: `grid-template-columns`
          e animavel, entao a tabela cresce junto em vez de dar um salto quando
          o painel some. `motion-reduce` respeita quem pediu menos animacao. */}
      <div
        className={cn(
          'grid gap-4 transition-[grid-template-columns] duration-300 ease-out motion-reduce:transition-none',
          painelCampos ? 'lg:grid-cols-[300px_1fr]' : 'lg:grid-cols-[40px_1fr]',
        )}
      >
        {/* ── Campos ──────────────────────────────────────────────── */}
        {!painelCampos ? (
        ...
        // A caixa encolhe, o conteudo nao: com `overflow-hidden` e a largura
