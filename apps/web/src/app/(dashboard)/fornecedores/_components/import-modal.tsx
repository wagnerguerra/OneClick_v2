...
const FORNECEDOR_COLUMNS: ColumnMapping[] = [
  { fileColumn: 'Razão Social', fieldName: 'razaoSocial', label: 'Razão Social', required: true },
  { fileColumn: 'CNPJ/CPF', fieldName: 'documento', label: 'Documento', required: true },
  { fileColumn: 'Nome Fantasia', fieldName: 'nomeFantasia', label: 'Nome Fantasia' },
  { fileColumn: 'Tipo', fieldName: 'tipoFornecedor', label: 'Tipo Fornecedor' },
  { fileColumn: 'Categoria', fieldName: 'categoria', label: 'Categoria' },
  { fileColumn: 'Email', fieldName: 'email', label: 'Email' },
  { fileColumn: 'Telefone', fieldName: 'telefone', label: 'Telefone' },
  { fileColumn: 'Contato', fieldName: 'contatoPrincipal', label: 'Contato Principal' },
...
export function ImportModal({ open, onClose, onSuccess }: ImportModalProps) {
  ...
  const requiredsMapped = FORNECEDOR_COLUMNS.filter(c => c.required).every(c => mappings.get(c.fieldName))
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      setFileData(data)
      setMappings(autoMapColumns(data.headers, FORNECEDOR_COLUMNS))
      setStep('mapping')
    } catch (e) { alerts.error('Erro', (e as Error).message) }
  }

  function handleMappingConfirm() { if (!fileData) return; setRows(parseWithMappings(fileData.rows, mappings, FORNECEDOR_COLUMNS)); setStep('preview') }
  ...
  async function handleImport() {
    ...
    try {
      ...
      const items: CreateFornecedorInput[] = validRows.map(r => ({
        ...
        tipoFornecedor: (TIPO_MAP[(r.data.tipoFornecedor ?? '').toLowerCase().trim()] ?? 'AMBOS') as CreateFornecedorInput['tipoFornecedor'],
        categoria: r.data.categoria ?? '',
        email: r.data.email ?? '',
        telefone: r.data.telefone ?? '',
        contatoPrincipal: r.data.contatoPrincipal ?? '',
        cep: r.data.cep ?? '',
      ...
      const result = await trpc.fornecedor.importBulk.mutate({ items })
      await alerts.success(result.errors.length ? 'Importação parcial' : 'Importação concluída', `${result.created} fornecedor(es) importado(s).`)
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader className="border-b border-border/60 bg-muted/30">
          <DialogTitle className="flex items-center gap-2.5">
            ...
            <div><span>Importar Fornecedores</span><DialogDescription className="mt-0.5">
        ...
        <div className="px-6 pb-2 max-h-[60vh] overflow-y-auto">
          {step === 'upload' && (
            <div className="space-y-4 py-2">
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="soft" size="sm" onClick={() => generateTemplate(FORNECEDOR_COLUMNS, 'template-fornecedores')}><Download className="h-4 w-4" />Template Excel</Button>
                <Button type="button" variant="soft" size="sm" onClick={() => generateTemplateCsv(FORNECEDOR_COLUMNS, 'template-fornecedores')}><Download className="h-4 w-4" />Template CSV</Button>
              </div>
          ...
          {step === 'mapping' && fileData && <div className="py-2"><ColumnMapper fileHeaders={fileData.headers} firstRow={fileData.firstRow} systemColumns={FORNECEDOR_COLUMNS} mappings={mappings} onMappingChange={handleMappingChange} /></div>}
          {step === 'preview' && (
