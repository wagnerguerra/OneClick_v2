...
/**
 * Casa o prefixo do arquivo com alguém do cadastro.
 *
 * Os arquivos vêm como "WAGNER - resumo-...". O primeiro nome basta, e a
 * comparação ignora acento e caixa — "JOAO" tem de achar "João Vitor". Sem
 * certeza, devolve vazio e a tela pede que o humano escolha.
 */
function acharAutor(prefixo: string, pessoas: Pessoa[]): string {
  ...
  const alvo = limpo(prefixo)
  if (!alvo) return ''
  const exato = pessoas.find(p => limpo(p.name).split(/\s+/)[0] === alvo)
  if (exato) return exato.id
  const contem = pessoas.filter(p => limpo(p.name).startsWith(alvo))
 ...
 * subir, e o que não casou com ninguém fica em vermelho esperando a escolha.
 */
  ...
  async function aceitar(lista: FileList | null) {
    ...
    try {
      ...
      for (const f of Array.from(lista)) {
        ...
        const corte = semExt.indexOf(' - ')
        const prefixo = corte > 0 ? semExt.slice(0, corte) : ''
        const titulo = (corte > 0 ? semExt.slice(corte + 3) : semExt).trim() || semExt
              ...
              <table className="w-full table-fixed">
                <thead>
                  <tr className="bg-muted/40 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                    <th className="w-[100px] px-3 py-2">Dia</th>
                    <th className="w-[190px] px-3 py-2">Autor</th>
                ...
                <tbody className="divide-y divide-border/60">
                  {achados.map(a => (
                    <tr key={a.chave} className={cn(!a.autorId && 'bg-amber-50/40 dark:bg-amber-950/10')}>
                      <td className="px-3 py-1.5 text-[12.5px] tabular-nums">
                      ...
                      <td className="px-3 py-1.5">
                        <select
                          ...
                          className={cn('h-7 w-full rounded border bg-background px-1 text-[12px]',
