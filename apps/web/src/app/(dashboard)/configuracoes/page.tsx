...
export default function ConfiguracoesPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setGroups(campos.groups as string[])
      const v: Record<string, string> = {}
      for (const c of configs as Array<{ key: string; value: string }>) v[c.key] = c.value
      setValues(v)
      if ((campos.groups as string[]).length > 0) setActiveGroup((campos.groups as string[])[0]!)
  ...
  async function handleSave() {
    setSaving(true)
    try {
      const fieldsToSave = activeGroup === 'Banco de Dados' ? allDbFields : activeFields
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="flex min-h-[500px]">
          ...
          <div className="w-[200px] shrink-0 border-r border-[rgba(0,0,0,0.08)] bg-[#f8f9fa] p-3 overflow-y-auto">
            <div className="space-y-1">
              {groups.map((group) => {
                ...
                return (
                  <button
                    key={group}
                    onClick={() => { setActiveGroup(group); if (group === 'Banco de Dados') setDbSubtab('postgresql') }}
                    className={cn(
                              ...
                              {showSavedList[dbSubtab] && (() => {
                                ...
                                function renderQueryItem(q: SavedQueryItem) {
                                  return (
                                    <div
                                      key={q.id}
                                      className="flex items-center gap-2 px-3 py-2 hover:bg-[#f8f9fa] border-b border-[rgba(0,0,0,0.04)] cursor-pointer group"
                                ...
                                return (
                                  <div className="absolute top-full left-0 mt-1 z-50 w-[350px] bg-white rounded-lg border shadow-lg" style={{ animation: 'fadeSlideIn 0.15s ease-out' }}>
                                    ...
                                    <div className="max-h-[300px] overflow-y-auto">
                                      ...
                                      {originals.length > 0 && (
                                        <>
                                          <div className="px-3 py-1.5 bg-[#f8f9fa] border-b border-[rgba(0,0,0,0.06)] flex items-center gap-1.5">
                                  ...
                                  <tbody>
                                    {filteredRows.length === 0 ? (
                                      ...
                                      filteredRows.map((row, ri) => (
                                        <tr key={ri} className="hover:bg-[#f8f9fa] border-b border-[rgba(0,0,0,0.04)]">
