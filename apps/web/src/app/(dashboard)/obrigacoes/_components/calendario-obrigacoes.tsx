...
/**
 * Tooltip CSS-only com os eventos do dia. Mesmo padrão da pill de feriados.
 */
...
export function CalendarioObrigacoes() {
  ...
  useEffect(() => {
    setLoading(true)
    ;(trpc as any).obrigacao.calendario.query({ ano })
      .then((res: Evento[]) => setEventos(res))
      .catch(() => setEventos([]))
      .finally(() => setLoading(false))
  }, [ano])

  // Agrupa por mês (0-11) → Map<dia, Evento[]>
  ...
  return (
    <div className="space-y-4">
      {/* Header do calendário: navegação de ano + legenda */}
