...
;(async () => {
  ...
  const [periodos] = await my.query('SELECT * FROM crp_ferias WHERE ativo = 1 ORDER BY id')
  ...
  for (const p of periodos) {
    ...
    sql.push(
      ...
      ` SELECT gen_random_uuid()::text, ${S(EMP)}, ${N(p.id)}, ${S(colab.id)}, ${S(colab.nome)},` +
      ...
      ` ${Number(p.pago) === 1 || pags.some(Boolean) ? 'true' : 'false'}, ${Number(p.historico) === 1 ? 'true' : 'false'},` +
  ...
  for (const e of eventos) {
    ...
    sql.push(
      ...
      ` ${S(ini)}::date, ${S(fim)}::date, ${S(String(e.descricao || '').trim() || null)}, ${S(reg.id)},` +
  ...
  for (const a of arquivos) {
    ...
    sql.push(
      ...
      ` SELECT gen_random_uuid()::text, ${subPeriodo(pai)}, ${N(a.Id)}, ${S(String(a.descricao || nomeFisico).trim())}, ${S('/api/upload/ferias-legado/' + nomeFisico)}, ${S(autor.id)},` +
